...
Shipped in v0.2. Externally reviewed before implementation; the review drove
reads-never-rebase, per-vault locking, no-loss record-conflict quarantine,
client-asserted attribution language, append-only-by-protocol wording,
dispute cardinality, the routing guardrail, and the two-team-vault cap.
...
- Records remain append-only and immutable; `supersedes` remains the only
  ...
  ([v0.4-auto-mode.md](v0.4-auto-mode.md)) adds `withdraw` (append-only
